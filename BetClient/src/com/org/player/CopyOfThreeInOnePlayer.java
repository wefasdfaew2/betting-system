...
public class CopyOfThreeInOnePlayer extends Thread implements MessageListener {
	...
	public static void main(String[] argv) {
		while (true) {
			try {
				...
				CopyOfThreeInOnePlayer client = new CopyOfThreeInOnePlayer(argv[0],
						argv[1], side);
				try {
	...
	public CopyOfThreeInOnePlayer(String username, String pass, OddSide side)
			throws JMSException {
		super();
		...
		logger = Logger.getLogger(CopyOfThreeInOnePlayer.class);
		this.util = new OddUtilities();
		this.side = side;
		this.current_map_odds = new HashMap<String, OddElement>();
	}

	public CopyOfThreeInOnePlayer(String acc, OddSide side) throws JMSException {
		super();
		this.p = new TopicPublisher();
		String[] a = acc.split(",");
		this.username = a[0];
	...
	public void placeBet(String script, float odd_value) {
		try {
			// logger.info("3in received");
			String key = script.split("'")[1];
			// from key we get to info
			String[] key_element = key.split("_");
			String info = key_element[0] + ",0," + key_element[1] + ","
					+ key_element[2] + ",0";
			// logger.info(info);
			...
			HtmlElement odd_element = ticket_page.createElement("a");
			odd_element.setAttribute("onclick",
					"javascript:return onOddsClick('info');".replaceAll("info",
							info));
			// logger.info(odd_element.asXml());
			ticket_page.appendChild(odd_element);
			odd_element.click();
